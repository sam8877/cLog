...
/** 通过登录页登录并等待跳转到后台 */
export async function login(page: Page, password = 'admin123', remember = false): Promise<void> {
...
export async function cleanupTestData(browser: Browser): Promise<void> {
  const ctx = await browser.newContext();
  const page = await ctx.newPage();
  try { await login(page); } catch { await ctx.close(); return; }
  ...
  for (const c of comments) {
    if (/^(访客|审核测试|筛选测试|垃圾测试|回复目标|串联评论)/.test(c.author) || /^(管理员回复|博主回复内容)/.test(c.body)) {
      await page.request.delete(`/api/comments/${c.id}`);
    }
  }
  // 文章
